...
config.imagehost = 'img.tuer.me';
var pichost = 'http://' + config.imagehost + '/images/',
picsize = [80, 150, 300, 500];

exports.drawUrl = function(str) {
	var regUrl = /(htt(p|ps):\/\/[A-Za-z\.\d\/\?\&\;\=\-\#_(\u4e00-\u9fa5)]+)/gi;
	return str.replace(regUrl, function(s1, s2, s3, s4) {
		var uri = s2;
...
exports.bacthImages = function(path, callback) {
	...
	function resizehandle(err) {
		if (err) {
			try {
				fs.unlinkSync(path);
				fs.unlinkSync(rootdir + '/public/images/80/' + picname);
...
exports.setCharset = function(req,res,data){
    var utf8Charset = {
...
exports.filterJson = function(data,map){
    var json = {};
